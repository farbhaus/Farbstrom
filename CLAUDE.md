...
- **Pinned ↔ grid is one FLIP, not three separate snaps** (`morphStage()` in `layout.ts`). It measures every tile, applies the DOM mutation, measures again, inverts the difference as a transform and releases it. For the duration, `body.stage-morphing` makes `#stage`'s padding change instant and mutes `tile-in` — otherwise the grid resizes *under* the travelling tiles and re-parented ones replay their entrance mid-jump, which is what made the switch feel rough. Reduced-motion gets the instant swap.
