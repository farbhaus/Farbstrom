...
- **To check "does the UI touch the image", audit rather than eyeball.** The mechanisms are `filter`, `opacity < 1`, `mix-blend-mode`, `backdrop-filter`, `mask` — **including a `border-radius` with a non-visible `overflow`, which is a mask** — a running `animation` (`tile-in` used to fade and scale the picture on every tile that appeared), an **inset** `box-shadow` (it paints inside the box, i.e. on the picture — the active-speaker ring is an outer shadow for this reason), and a translucent background or gradient painted over it. Walk the ancestor chain from the `<video>` to `<html>`, then every element whose rect overlaps the picture — in pinned *and* grid, chrome shown *and* faded, scopes open. Enumerating selectors by hand is what let the scopes window keep its blur through a whole "fix"; enumerate *mechanisms* against the live DOM instead.
- **Chrome that floats over the picture is `--pill-bg-opaque`, not `--pill-bg`.** 72% translucent chrome blends the image into itself. `#scopes-window` is on that list too — a translucent panel blends the picture into the surface you read measurements off. Note the limit: headless browsers render video in software, so this catches CSS causes but **cannot** exercise GPU compositing-path changes, which is the class the #248 backdrop-filter bug belonged to. Those need a real browser and a SMPTE ingest.
