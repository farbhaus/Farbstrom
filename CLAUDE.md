...
- **To check "does the UI touch the image", audit rather than eyeball.** The complete set of CSS mechanisms that can alter rendered colour is `filter`, `opacity < 1`, `mix-blend-mode`, `backdrop-filter`, `mask`, and a translucent background or gradient painted over the picture. Walk the ancestor chain from the `<video>` to `<html>` for the first five, then every element whose rect overlaps the picture for all six — in pinned *and* grid, chrome shown *and* faded, scopes open. Note the limit: headless browsers render video in software, so this catches CSS causes but **cannot** exercise GPU compositing-path changes, which is the class the #248 backdrop-filter bug belonged to. Those need a real browser and a SMPTE ingest.
